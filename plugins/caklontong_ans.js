...
handler.before = async function (m) {
    ...
    if (/^(me)?nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.caklontong[id][1].jawaban))}`.trim(), '© Made by Rey ❤️', 'CAK LONTONG', '.caklontong').then(() => { delete this.caklontong[id] })
    }
