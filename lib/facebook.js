...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data == 'error') return resolve({ creator: '@Made By Rey', status: false, message: 'tida dapat diunduh' })
                resolve({ creator: '@Made by Rey', status: true, thumb: data.thumbnail, data: data.links })
            })
    })
}
